...
namespace demonware
{
	namespace
	{
		IN_ADDR detour_server_addr{};

		namespace io
		{
			int getaddrinfo_stub(const char* name, const char* service, const addrinfo* hints, addrinfo** res)
			{
				if (!strcmp(name, "ops3-pc-auth3.prod.demonware.net"))
				{
					printf("^1Detouring '%s' with port\n", name);
					...
					in_addr->sin_addr.s_addr = detour_server_addr.s_addr; // DETOUR_SERVER_ADDRESS
				...
				else if (!strcmp(name, "ops3-pc-lobby.prod.demonware.net"))
				{
					printf("^1Detouring '%s'\n", name);
				...
				// Fallback to the original getaddrinfo if no detour is found
	...
	struct component final : generic_component
	{
		...
		{
			//LOGIC TO DETOUR AND SET IP -- NO JSON CONFIG HERE 
			std::string server_ip_str = "127.0.0.1";
			IN_ADDR addr_resolve{};
			if (inet_pton(AF_INET, server_ip_str.data(), &addr_resolve) == 1) {
				detour_server_addr = addr_resolve;
			}
			else {
				detour_server_addr.s_addr = 0x100007F;
				printf("[ network ]: wrong dw server address specified: '%s'\n", server_ip_str.data());
			}
